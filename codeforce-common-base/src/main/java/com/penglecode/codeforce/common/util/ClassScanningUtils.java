...
public class ClassScanningUtils {
	...
	public static Set<String> scanClassNames(String... basePackages) {
		...
		return scanClassNames(scanningProvider, basePackages);
	}
	...
	private static Set<String> scanClassNames(ClassPathScanningCandidateComponentProvider scanningProvider, String... basePackages) {
	...
	public static class JavaCoreResourceClassLoader extends ClassLoader {
		...
		protected Enumeration<URL> findResources(String name) throws IOException {
			if(isJavaCorePackageResource(name)) {
				URL url = getJavaCorePackageResource(name);
				if(url != null) {
					return CollectionUtils.asEnumeration(Collections.singleton(url).iterator());
				}
			}
		...
		protected boolean isJavaCorePackageResource(String name) {
			return StringUtils.isNotEmpty(name) && name.startsWith("java/") && !name.endsWith(".class");
		}
