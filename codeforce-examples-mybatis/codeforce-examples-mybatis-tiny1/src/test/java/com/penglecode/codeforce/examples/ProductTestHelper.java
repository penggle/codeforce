...
import com.penglecode.codeforce.examples.mybatis.domain.enums.ProductTypeEnum;
...
public class ProductTestHelper {
     ...
     * 生成单个测试数据
     */
    public static ProductAggregate genTestProduct4Create() {
        String nowTime = DateTimeUtils.formatNow();
        ProductAggregate product = new ProductAggregate();
        product.setProductName("24期免息【当天发】Huawei/华为Mate40 5G手机官方旗舰店50pro直降mate40e官网30正品4G鸿蒙正品30全网通");
        product.setProductUrl("https://detail.tmall.com/item.htm?id=633658852628");
        product.setProductTags("华为手机 5G mate40pro");
        product.setProductType(ProductTypeEnum.PHYSICAL_PRODUCT.getTypeCode());
        product.setAuditStatus(ProductAuditStatusEnum.WAIT_AUDIT.getStatusCode());
        product.setOnlineStatus(ProductOnlineStatusEnum.OFFLINE.getStatusCode());
        product.setShopId(111212422L);
        product.setRemark("当天发货 保修3年 送影视会员 咨询客服");
        ...
        for (List<ProductSaleSpec> cartesian : cartesians) {
            ...
            productSaleStock.setProductId(product.getProductId());
     ...
     * 生成单个测试数据
     */
    public static ProductAggregate genTestProduct4Modify(Long productId) {
        String nowTime = DateTimeUtils.formatNow();
        ProductAggregate product = new ProductAggregate();
        product.setProductId(productId);
        product.setProductName("24期免息【当天发】Huawei/华为Mate40 5G手机官方旗舰店50pro直降mate40e官网30正品4G鸿蒙正品30全网通AAA");
        product.setProductUrl("https://detail.tmall.com/item.htm?id=633658852628");
        product.setProductTags("华为手机 5G mate40proAAA");
        product.setProductType(ProductTypeEnum.PHYSICAL_PRODUCT.getTypeCode());
        product.setAuditStatus(ProductAuditStatusEnum.WAIT_AUDIT.getStatusCode());
        product.setOnlineStatus(ProductOnlineStatusEnum.OFFLINE.getStatusCode());
        product.setShopId(111212422L);
        product.setRemark("当天发货 保修3年 送影视会员 咨询客服AAA");
        product.setCreateTime(nowTime);
        product.setUpdateTime(nowTime);
    ...
    public static ProductAggregate getTestProductFromJD(Long productId, Long sellPrice) throws Exception {
        ...
        ProductAggregate product = new ProductAggregate();
        product.setProductId(productId);
        product.setProductName(productJsonNode.get("name").asText());
        product.setProductUrl(jdDetailUrl);
        product.setProductTags(productJsonNode.get("catName").toString());
        product.setProductType(ProductTypeEnum.PHYSICAL_PRODUCT.getTypeCode());
        product.setAuditStatus(ProductAuditStatusEnum.WAIT_AUDIT.getStatusCode());
        product.setOnlineStatus(ProductOnlineStatusEnum.OFFLINE.getStatusCode());
        product.setShopId(productJsonNode.get("shopId").asLong());
        ...
        product.setProductExtra(productExtra);
        product.setProductSaleSpecs(allProductSaleSpecs);
        product.setProductSaleStocks(allProductSaleStocks);
        return product;
    }
    ...
    public static void main(String[] args) throws Exception {
        ProductAggregate testProduct = getTestProductFromJD(100018640796L, 229900L);
